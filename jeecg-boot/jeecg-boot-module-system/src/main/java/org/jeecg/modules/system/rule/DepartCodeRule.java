    ...
    public Object execute(JSONObject params, JSONObject formData) {
        ...
        if (StringUtil.isNullOrEmpty(parentId)) {
            ...
            if (departList == null || departList.size() == 0) {
                ...
                newOrgCode = String.valueOf(Integer.parseInt(oldOrgCode) + 1);
            }
            ...
            if (parentList == null || parentList.size() == 0) {
                // 直接生成当前的部门编码并返回
                newOrgCode = parentCode + "-1";
            } else { //处理有同级部门的情况
                // 获取同级部门的编码,利用工具类
                String subCode = parentList.get(0).getOrgCode();
                // 返回生成的当前部门编码
                int index = subCode.lastIndexOf("-");
                newOrgCode = subCode.substring(0, index + 1) + (Integer.parseInt(subCode.substring(index + 1)) + 1);
            }
        }
        // 返回最终封装了部门编码和部门类型的数组
